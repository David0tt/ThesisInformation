#!/usr/bin/env python3
# copied from https://github.com/jk-ethz/franka_lock_unlock with minor modifications and addition to shut down the robot
# 
# required install:
#
...
# ### Usage ###
# #############
# To unlock the joints and keep them unlocked and expose FCI (should lock the joints again after closing this terminal):
#
...
class FrankaLockUnlock:
    def __init__(self, hostname: str, username: str, password: str, protocol: str = 'https', relock: bool = False):
        ...
        self._token_id = None
        self._relock = relock
        self._robot_is_shutdown = False
        atexit.register(self._cleanup)

    def _cleanup(self):
        print("Cleaning up...")
        if self._robot_is_shutdown:
            print("Nothing to clean up, since robot is shut down")
            print(80 * "#")
            print("Wait until the lights are off and the fans stopped spinning! \nThen turn off the power.")
            print(80 * "#")
            return
        if self._relock:
    ...
    def _shutdown_robot(self):
        ...
        assert self._token is not None, "Control token is required to shut down the robot."
        try:
            shutdown_request = self._session.post(
                urljoin(self._hostname, '/admin/api/shutdown'),
                headers={'X-Control-Token': self._token}
            )
            self._robot_is_shutdown = True
            assert shutdown_request.status_code == 200, "Error shutting down the robot."
            print("Successfully shut down the robot.")
        except requests.exceptions.ConnectionError as e: # or ConnectionRefusedError or as 
            print("Robot shut down successfully, but the connection to the server was lost.")
            self._robot_is_shutdown = True  # Set a flag to indicate the robot is shut down.
    ...
    def run(self, unlock: bool = False, force: bool = False, wait: bool = False, request: bool = False, persistent: bool = False, fci: bool = False, home: bool = False, shutdown: bool = False) -> None:
        """
        Run the lock/unlock and other operations.
        """
                ...
                try:
                    ...
                    for _ in range(20) if request else count():
                        if (not wait and not request) or self._is_active_token():
                            ...
                            if not self._robot_is_shutdown:
                        ...
                        if request:
                            ...
                            print('Please confirm the request message in the web interface as the logged-in user.')
                        sleep(1)
                    # In case physical access was not confirmed, try again
                    self._release_token()
                finally:
                    if not persistent and not self._robot_is_shutdown:
                        self._release_token()
        finally:
            if not persistent and not self._robot_is_shutdown:
                self._logout()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
                                     ...
                                     description = 'Lock, unlock, or shut down the Franka Emika Panda robot arm programmatically.',
    ...
    parser.add_argument('-s', '--shutdown', action='store_true', help='Shutdown the robot.')
